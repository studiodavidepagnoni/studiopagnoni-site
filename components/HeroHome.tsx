...
export function HeroHome() {
  ...
  const useVideo = videoOk && !reduceMotion && !isMobile;
  const line2Parts = s.line2.split(" · ").map((p) => p.trim()).filter(Boolean);

  return (
    <section className="relative isolate min-h-[96svh] overflow-hidden sm:min-h-[92vh] md:min-h-[620px]" aria-label="Introduzione">
      <div
      ...
      <div className="pointer-events-none absolute inset-0 z-[-10]">
        {/* Background fisso: video su desktop, immagine statica su mobile/reduced-motion */}
        {useVideo ? (
          <video
      ...
      <div className="relative z-20 mx-auto flex min-h-[96svh] max-w-[1200px] flex-col justify-center gap-6 px-4 pb-14 pt-24 sm:min-h-[92vh] sm:gap-8 sm:px-6 sm:pb-16 sm:pt-28 md:min-h-[620px] md:pb-20 md:pt-32">
        {/* Testo ciclico: key=i forza il re-mount e la transizione ad ogni slide */}
        <motion.div
          key={i}
          className="max-w-[min(100%,40rem)] text-left"
          initial={{ opacity: 0, y: 14 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.55, ease: subtleEase }}
        >
          <p
          ...
          >
            {line2Parts.map((part, idx) => (
              <span key={`${part}-${idx}`}>
                ...
                {idx < line2Parts.length - 1 ? <span className="mx-2 text-white/40">·</span> : null}
              </span>
            ))}
          </p>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:mt-10 sm:flex-row sm:items-center sm:gap-4">
            ...
            <Link href={s.ctaHref} className={`${ui.btnHeroGhost}`}>
              {s.ctaLabel}
            </Link>
          </div>
        </motion.div>
